...
Select.prototype.setData = function (data) {
  ...
  for (var i = 0, l = data.length; i < l; i++) {
    var o = data[i];
    var el = domify('<li class="scroll-select-item" data-index="' + i + '" data-value="' + o.text + '">' + o.text + '</li>');
...
Select.prototype.onscrollend = function () {
  ...
  var y = is.y;
  if (y > 0) {
  	return is.scrollTo(0, 300);
  }
  if (y < is.minY) {
  	return is.scrollTo(is.minY, 300);
  }
...
/**
 * Hides values from select
 *
 * @public
 * @param {Array|String} values
 */
Select.prototype.filter = function (values) {
	if (arguments.length === 0) {
		var _items = document.querySelectorAll('.scroll-select-item');
